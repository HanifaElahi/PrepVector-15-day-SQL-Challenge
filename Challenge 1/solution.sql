...
SELECT 
  ROUND((CAST(count(distinct u.user_id) AS DOUBLE) / (SELECT count(distinct user_id) FROM users))*100, 2) AS percentage
FROM users u
LEFT JOIN events_data ed ON ed.user_id = u.user_id
